...
import { buildPublicFrontpageJobsPayload } from "@/lib/public-frontpage-jobs";
...
export async function GET(request: Request) {
  try {
    ...
    sourceUrl.pathname = sourceUrl.pathname.replace(/\/frontpage\/?$/, "");
    sourceUrl.searchParams.delete("callback");

    const sourceHeaders = new Headers(request.headers);
    sourceHeaders.delete("if-none-match");
    const sourceResponse = await getJobs(
      new Request(sourceUrl.toString(), { method: "GET", headers: sourceHeaders })
    );

    if (!sourceResponse.ok) {
      const response = new NextResponse(await sourceResponse.text(), {
        status: sourceResponse.status,
        headers: {
          "Content-Type": sourceResponse.headers.get("content-type") ?? "application/json",
        },
      });
      applyPublicHeaders(response.headers);
    ...
    const payload = buildPublicFrontpageJobsPayload(source, requestUrl.origin);
    ...
    const message = error instanceof Error ? error.message : "Unable to load frontpage jobs.";
