(() => {
  try {
    document.documentElement.setAttribute("data-linas-jobs-mount", "v3");
  } catch {
    // ignore
  }
  ...
  function readGlobalConfig() {
    try {
      const value = window.LinasJobsEmbedConfig;
  ...
  const apiBase =
    normalizeBase(
      readDataAttribute(scriptEl, "api-base") ||
        (typeof globalConfig.apiBase === "string" ? globalConfig.apiBase : "") ||
        (typeof globalConfig.api_base === "string" ? globalConfig.api_base : "")
    ) || scriptUrl.origin;
  const targetSelector = (
    readDataAttribute(scriptEl, "target") ||
    (typeof globalConfig.target === "string" ? globalConfig.target : "") ||
    (typeof globalConfig.targetSelector === "string"
      ? globalConfig.targetSelector
      : "")
  ).trim();
  const debugEnabled = (() => {
    const configDebug =
      typeof globalConfig.debug === "string" || typeof globalConfig.debug === "number"
        ? String(globalConfig.debug)
        : typeof globalConfig.debug === "boolean"
        ? globalConfig.debug
          ? "true"
          : "false"
        : "";
    const raw = (readDataAttribute(scriptEl, "debug") || configDebug)
      .trim()
      .toLowerCase();
  ...
  function boot() {
    ...
    debugLog("boot targets", targets.length, targets);
    for (const target of targets) mountInto(target);
    return true;
  }

  try {
    window.LinasJobsMount = {
      version: "v3",
      boot,
    };
  } catch {
  ...
  function mountInto(container) {
    try {
      container.setAttribute("data-linas-jobs-mounted", "1");
      container.setAttribute("data-linas-jobs-api-base", apiBase);
    } catch {
      // ignore
    }
    debugLog("mountInto", { apiBase, targetSelector, container });
